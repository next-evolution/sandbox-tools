...
./
  ...
  setup/                       ← DB 再構築後に 1 回だけ実行するもの
    ...
    200_master_data/
      step02〜13.bru           ← マスターデータ・バーデータ・ZigZag 登録
...
| `setup/200_master_data/register.sh`     | マスター・バーデータ・ZigZag の一括登録 | ✅   |
| `setup/200_master_data/step02〜13.bru`  | 各登録ステップ（CLI から呼び出し）    | ✅   |
...
# [2] マスターデータ・バーデータ・ZigZag 登録（Step02 から全実行）
./setup/200_master_data/register.sh

# [2'] 任意ステップから再開
./setup/200_master_data/register.sh --from-step 9
...
[Step08]   ZigZag 初期データ投入               直接 DB INSERT                     ← data/zigzag/*.csv
[Step09]   バーデータ CSV インポート           POST:/api/v1/fx/bar-data/import-csv ← curl（bar_15m/1h/4h/1d）
[Step10]   経済指標登録                        POST:/api/v1/fx/economic-indicator  ← economic_indicator.csv をループ
[Step11]   経済指標データ登録                  POST:/api/v1/fx/economic-indicator-data ← economic_indicator_data.csv をループ
[Step12]   経済指標データ テキストインポート   POST:/api/v1/fx/economic-indicator-data/import-text ← curl（2026_03_H.txt + 2026_03_M.txt）
[Step13]   ZigZag 生成                         POST:/api/v1/fx/zigzag/generate    ← 全シンボル × 4 barType（15M/1H/4H/1D）
```

- Step07・Step08 は API を経由せず `mysql` コマンドで直接 INSERT する
- Step09・Step12 はファイルアップロードのため `bru run` を使わず curl で直接実行する
  - bru run では `@file({{変数}})` の変数展開が機能せず 500 エラーになるため
- Step13 の ZigZag 生成パラメータ: `depth=3`・`barDateTime=2026-03-01T00:00:00+09:00`・`loadSize=1000`
...
# ZigZag 生成のみ実行
./setup/200_master_data/register.sh --from-step 13
```
